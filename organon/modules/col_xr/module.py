...
legacy (voir la discussion dans `organon/modules/gbif/module.py` : les deux référentiels peuvent
diverger, comparer les facettes proposées revient à l'utilisateur).
...
class ColXrModule(TaxonomyModule):
    ...
    def render_bioref(self, struct: Struct) -> str | None:
        ...
        # {{GBIF 2}} n'existe pas encore comme modèle Wikipédia dédié : gabarit provisoire choisi
        # avec l'utilisateur (voir organon/modules/gbif/module.py) en attendant qu'un modèle
        # propre à la Catalogue of Life Extended Release soit créé.
        return f"{{{{GBIF 2 | {data['id']} | {cible}{sup}{nv} | consulté le={cdate} }}}}"
